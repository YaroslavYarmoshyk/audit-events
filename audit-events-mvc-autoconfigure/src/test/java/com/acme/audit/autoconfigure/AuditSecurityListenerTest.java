...
class AuditSecurityListenerTest {
    ...
    void interactiveLoginRecordsLoginAttributedToEventPrincipal() {
        listener(true, true).onInteractiveAuthenticationSuccess(interactiveLogin(TestData.ALICE));
    ...
    void disabledFlagsSuppressEvents() {
        ...
        listener.onInteractiveAuthenticationSuccess(interactiveLogin(TestData.ALICE));
    ...
    private AuditSecurityListener listener(boolean login, boolean logout) {
        return new AuditSecurityListener(publisher, login, logout);
    }

    private InteractiveAuthenticationSuccessEvent interactiveLogin(String user) {
        return new InteractiveAuthenticationSuccessEvent(TestDataFactory.principal(user), getClass());
    }
}
